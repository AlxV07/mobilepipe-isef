...
(click image to view larger PNG in repo)
...
* `client` - Client Layer: mobile (iOS) layer using Swift + Metal Performance Shaders (MPSGraph for hardware accelerated ML operations; we chose MPSGraph over mlx Swift due to its more direct control over memory and operation flow on the iOS Neural Engine and GPU (mlx is built on top of MPSGraph; mlx is generally faster than the mps backend integrated through Python on Mac computers, but on iOS MPSGraph runs directly native through Metal Shading Language GPU kernels). In future versions we may switch to heavier mlx Swift reliance due to rigidness in MPSGraph and dynamic graph construction in mlx).
